...
contract Owned {
    ...
    modifier onlyOwner(){ 
        if(msg.sender != owner){
            revert("this function can invoke owner");
        }
        _;
    }
...
contract Members is Owned {
    ...
    function editStatus(uint _index, string memory _name, uint256 _times, uint256 _sum, int8 _rate) public onlyOwner {
    ...
    function getCashbackRate(address _member) public view returns (int8 rate) {
        uint256 statusIndex = tradingHistory[_member].statusIndex;
        rate = status[statusIndex].rate;
    }
}
...
contract OreOreCoin is Owned {
    ...
    function transfer(address _to, uint256 _value) public payable{
        if (balanceOf[msg.sender] < _value) revert("invalid value");
        if (balanceOf[_to] + _value < balanceOf[_to]) revert("invalid value");
